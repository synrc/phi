...
data ExternsFile = ExternsFile
  ...
  , objAccs :: [(Ident, PSString)]
  , objs :: [(Ident,[(PSString, Qualified Ident)])]
  } deriving (Show)
...
moduleToExternsFile :: [(Ident, PSString)] -> 
                       [(Ident,[(PSString, Qualified Ident)])] ->
                       Module -> Environment -> ExternsFile
moduleToExternsFile _ _ (Module _ _ _ _ Nothing) _ = internalError "moduleToExternsFile: module exports were not elaborated"
moduleToExternsFile oa ob (Module ss _ mn ds (Just exps)) env = ExternsFile{..}
  where
