defmodule Phi.Desugar do
  ...
  defp group_decls([%AST.DeclValue{name: name} = d | rest]) when name != "_pat" do
    ...
    merged =
      if same == [] do
        ...
        branches =
          Enum.map(all_eqs, fn eq ->
            pat =
              case length(eq.binders) do
                1 -> hd(eq.binders)
                _ -> %AST.ExprTuple{elems: eq.binders}
              end

            body =
              if eq.guards && length(eq.guards) > 0, do: desugar_guards(eq.guards), else: eq.expr
        ...
        %AST.DeclValue{
          name: name,
          binders: new_binders,
          expr: %AST.ExprCase{exprs: [case_target], branches: branches},
          guards: nil
        }
      end
  ...
  defp desugar_expr(%AST.ExprRecordUpdate{base: base, fields: fields}) do
    %AST.ExprRecordUpdate{
      base: desugar_expr(base),
      fields: Enum.map(fields, fn {n, e} -> {n, desugar_expr(e)} end)
    }
  end

  defp desugar_expr(%AST.ExprList{elems: elems, tail: tail}) do
    %AST.ExprList{
      elems: Enum.map(elems, &desugar_expr/1),
      tail: if(tail, do: desugar_expr(tail), else: nil)
    }
  end

  defp desugar_expr(%AST.ExprTuple{elems: elems}) do
    %AST.ExprTuple{elems: Enum.map(elems, &desugar_expr/1)}
  end
  ...
  defp desugar_guards([{guard, body} | rest]) do
    %AST.ExprIf{
      cond: desugar_expr(guard),
      then_br: desugar_expr(body),
      else_br: desugar_guards(rest)
    }
  end
end
