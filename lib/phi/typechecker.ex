defmodule Phi.Typechecker do
  ...
  defmodule Env do
    ...
    def all_keys(%Env{bindings: bindings}) do
      Map.keys(bindings)
    end

    def merge(%Env{} = e1, %Env{} = e2) do
      %Env{
        bindings: Map.merge(e1.bindings, e2.bindings),
        aliases: Map.merge(e1.aliases, e2.aliases),
        classes: Map.merge(e1.classes, e2.classes),
        instances:
          Map.merge(e1.instances, e2.instances, fn _k, v1, v2 -> Enum.uniq(v1 ++ v2) end),
        member_to_class: Map.merge(e1.member_to_class, e2.member_to_class),
        term_aliases: Map.merge(e1.term_aliases, e2.term_aliases),
        module_aliases: Map.merge(e1.module_aliases, e2.module_aliases),
        class_to_members:
          Map.merge(e1.class_to_members, e2.class_to_members, fn _k, v1, v2 ->
            Enum.uniq(v1 ++ v2)
          end)
      }
    end
  ...
  def build_env(%AST.Module{name: mod_name, declarations: decls}, env) do
    ...
    Enum.reduce(decls, env, fn
      ...
      %AST.DeclClass{name: name, args: args, members: members}, acc ->
        ...
        acc_with_class_metadata = %{
          acc
          | classes: Map.put(acc.classes, name, %{args: arg_names, members: members})
        }

        # 2. Populate member_to_class and class_to_members
        member_names =
          Enum.map(members, fn %AST.DeclTypeSignature{name: m_name} -> "#{mod_name}.#{m_name}" end)
        ...
        acc_with_class_info = %{
          ...
          | member_to_class:
              Enum.reduce(
                members,
                acc_with_class_metadata.member_to_class,
                fn %AST.DeclTypeSignature{name: m_name}, acc_m ->
                  acc_m |> Map.put(m_name, name) |> Map.put("#{mod_name}.#{m_name}", name)
                end
              ),
            class_to_members:
              Map.put(acc_with_class_metadata.class_to_members, name, member_names)
        }
          ...
          %AST.DeclTypeSignature{name: m_name, type: m_type}, env_acc ->
            ...
            constraint =
              if args == [] do
                ...
                Enum.reduce(args, %AST.TypeConstructor{name: name}, fn %AST.TypeVar{
                                                                         name: arg_name
                                                                       },
                                                                       f ->
                  %AST.TypeApp{func: f, arg: %AST.TypeVar{name: arg_name}}
                end)
              end
      ...
      acc ->
        ...
        type_suffixes =
          Enum.map(itypes, fn
            %Phi.Type.TApp{func: %Phi.Type.TApp{func: %Phi.Type.TCon{name: "Tuple"}}} -> "Tuple"
            %Phi.Type.TApp{func: %Phi.Type.TApp{func: %Phi.Type.TCon{name: "->"}}} -> "Fun"
            %Phi.Type.TApp{func: %Phi.Type.TCon{name: n}} -> n
            %TCon{name: n} -> n
            %TVar{id: id} -> "v#{id}"
            _ -> "Any"
          end)
        ...
        new_instances =
          Map.update(acc.instances, class_name, [instance_info], fn existing ->
      ...
      %AST.DeclData{name: data_name, args: args, constructors: constructors}, acc ->
        ...
        ret_type =
          Enum.reduce(type_vars, %TCon{name: data_name}, fn tv, acc_type ->
        ...
        Enum.reduce(constructors, acc, fn {c_name, c_args}, env_acc ->
          ...
          con_type =
            Enum.reduce(Enum.reverse(c_arg_types), ret_type, fn t_arg, acc_t ->
          ...
          env_with_ctor =
            Env.extend(
              ...
              erl_mod,
              c_name,
              %Forall{vars: var_names, type: con_type},
              mod_name
            )
      ...
      %AST.DeclNewtype{name: nt_name, args: args, constructor: ctor_name, type: wrapped_type},
      acc ->
        type_vars =
          Enum.map(args, fn
            %AST.TypeVar{name: n} -> %TVar{id: n}
            n when is_binary(n) -> %TVar{id: n}
          end)

        ret_type =
          Enum.reduce(type_vars, %TCon{name: nt_name}, fn tv, acc_type ->
            %TApp{func: acc_type, arg: tv}
          end)
      ...
      %AST.DeclImport{module: m, alias: a, import_list: items}, acc ->
        ...
        Enum.reduce(items || [], acc1, fn
          ...
          {:operator, op}, acc_in ->
            ...
            existing_valid =
              existing &&
                (Map.has_key?(acc_in.bindings, existing) || String.starts_with?(existing, "#{m}."))
          ...
          {:class, class_name}, acc_in ->
            ...
            env_with_class_members =
              case Map.get(acc.classes, qualified_name) do
                nil ->
                  acc_in
                ...
                %{members: members} ->
                  %{
                    acc_in
                    | member_to_class:
                        Enum.reduce(members, acc_in.member_to_class, fn
                          %AST.DeclTypeSignature{name: m_name}, acc_m ->
                            acc_m
                            |> Map.put(m_name, qualified_name)
                            |> Map.put("#{class_name}.#{m_name}", qualified_name)

                          # Handle cases where members might be tuples (e.g., from parsing)
                          {m_name, _}, acc_m ->
                            acc_m
                            |> Map.put(m_name, qualified_name)
                            |> Map.put("#{class_name}.#{m_name}", qualified_name)
            ...
            Enum.reduce(members_from_class, env_with_class_members, fn member_full_name,
                                                                       acc_inner ->
              ...
              %{
                acc_inner
                | term_aliases:
                    Map.put(acc_inner.term_aliases, member_base_name, member_full_name)
              }
            end)
  ...
  # Using string ID for bound variables initially
  def ast_to_type(%AST.TypeVar{name: name}, _env), do: %TVar{id: name}
  ...
  def ast_to_type(%AST.TypeConstrained{constraints: [c | rest], type: t}, env) do
    ...
    inner =
      if rest == [],
        do: ast_to_type(t, env),
        else: ast_to_type(%AST.TypeConstrained{constraints: rest, type: t}, env)

    case flatten_type_app(c, []) do
      [%AST.TypeConstructor{name: name} | args] ->
        %Phi.Type.TConstrained{
          class_name: name,
          args: Enum.map(args, &ast_to_type(&1, env)),
          type: inner
        }
  ...
  defp do_infer(env, state, %AST.ExprLam{binder: binder, body: body}) do
    ...
    case infer_binder(env, state, binder) do
      {:ok, type_binder, bound_vars, state2} ->
        ...
        env2 =
          Enum.reduce(bound_vars, env, fn {name, t}, acc_env ->
  ...
  defp do_infer(env, state, %AST.ExprLet{
         bindings: [%AST.DeclValue{name: name, expr: expr}],
         body: body
       }) do
    case do_infer(env, state, expr) do
      {:ok, t_expr, state2} ->
  ...
  defp do_infer(env, state, %AST.ExprCase{exprs: target_exprs, branches: branches}) do
    ...
    {t_targets, state2} =
      Enum.reduce(target_exprs, {[], state}, fn expr, {ts, st} ->
    ...
    # 3. Infer each branch
    state4 =
      Enum.reduce(branches, state3, fn {patterns, body}, st_acc ->
        # Patterns might bind new variables
        {t_patterns, bound_vars, st_next} =
          Enum.reduce(patterns, {[], %{}, st_acc}, fn pat, {t_pats, b_vars, st_p} ->
        ...
        st_next2 =
          Enum.zip(t_targets, t_patterns)
          |> Enum.reduce(st_next, fn {t_target, t_pat}, st_unify ->
  ...
  defp do_infer(_env, _state, expr),
    do: {:error, "Unsupported expression for inference: #{inspect(expr)}"}
  ...
  defp infer_binder(env, state, %AST.BinderConstructor{name: name, args: args}) do
    ...
    case lookup_result do
      {:ok, scheme} ->
        ...
        {arg_types, bound_vars, state3} =
          Enum.reduce(args, {[], %{}, state2}, fn arg, {t_args, b_vars, st} ->
        ...
        t_expected =
          Enum.reduce(Enum.reverse(arg_types), t_ret, fn arg_t, acc ->
            Phi.Type.arrow(arg_t, acc)
          end)
  ...
  defp do_unify(
         %Phi.Type.TRowExtend{label: l1, type: t1, rest: r1},
         %Phi.Type.TRowExtend{} = row2,
         state
       ) do
  ...
  defp do_unify(
         %Phi.Type.TConstrained{class_name: c1, args: a1, type: t1},
         %Phi.Type.TConstrained{class_name: c2, args: a2, type: t2},
         state
       ) do
    if c1 == c2 and length(a1) == length(a2) do
      state2 =
        Enum.zip(a1, a2)
        |> Enum.reduce(state, fn {arg1, arg2}, st ->
  ...
  defp bind(id, type, state) do
    if occurs_check(id, type) do
      ...
      composed =
        Map.new(state.subst, fn {k, v} -> {k, apply_subst(new_subst, v)} end)
        |> Map.put(id, type)
  ...
  defp apply_subst(subst, %Phi.Type.TConstrained{class_name: c, args: args, type: t}) do
    %Phi.Type.TConstrained{
      class_name: c,
      args: Enum.map(args, &apply_subst(subst, &1)),
      type: apply_subst(subst, t)
    }
  end
  ...
  defp free_vars(%Forall{vars: vars, type: t}),
    do: MapSet.difference(free_vars(t), MapSet.new(vars))

  defp free_vars(%Phi.Type.TRowEmpty{}), do: MapSet.new()

  defp free_vars(%Phi.Type.TRowExtend{type: t, rest: r}),
    do: MapSet.union(free_vars(t), free_vars(r))
  ...
  defp instantiate(%Forall{vars: vars, type: t}, state) do
    {subst, state2} =
      Enum.reduce(vars, {%{}, state}, fn var, {s, st} ->
