...
encodeRebuildErrors = toJSON . map encodeRebuildError . P.runMultipleErrors
  where
    ...
    insertTSCompletions name idents fields (Aeson.Object value) =
      Aeson.Object
        (KM.insert (K.fromString "pursIde")
         (object [ K.fromString "name" .= name
                 , K.fromString "completions" .= (ordNub (map identCompletion idents ++ map fieldCompletion fields))
                 ]) value)
...
prettyPrintTypeSingleLine :: Show a => P.Type a -> Text
