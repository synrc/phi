...
convertType fileName = go
  ...
  go = \case
    ...
    TypeList _ a -> do
      let
        a' = go a
        arr' = Env.tyList $> Pos.nullSourceAnn
...
reBuild [x] s = AST.ListBinder [AST.ParensInBinder x] s
reBuild (x:xs) s = AST.ListBinder [AST.ParensInBinder x] (reBuild xs s)
