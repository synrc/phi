...
moduleToCoreFn env (A.Module modSS coms mn decls (Just exps)) =
  ...
  exprToCoreFn ss com ty (A.Receive (Just (e1, e2)) alts) =
    Receive (ss, com, ty, Nothing) (Just (e1, (exprToCoreFn ss [] Nothing e2))) (fmap (altToCoreFn ss) alts)
  exprToCoreFn ss com ty (A.Receive Nothing alts) =
    Receive (ss, com, ty, Nothing) Nothing (fmap (altToCoreFn ss) alts)
  ...
  exprToCoreFn ss com _ (A.TypeClassDictionaryConstructorApp name (A.Literal _ (A.ObjectLiteral vs))) =
    let args = fmap (\(a,b) -> (a,exprToCoreFn ss [] Nothing $ b)) $ sortBy (compare `on` fst) vs
    in Literal (ss, com, Nothing, Nothing) $ A.ObjectLiteral args
