defmodule Phi.Layout do
  ...
  defp do_resolve([token | _rest] = tokens, acc, ctx, depth) do
    line = elem(token, 1)
    col = elem(token, 2)
  ...
  defp do_resolve_token([{:where, line, col} | rest], acc, ctx, depth) do
    ...
    case rest do
      [] ->
        do_resolve(
          [],
          [{:left_brace, line, col + 1} | acc],
          [{:where, col + 1, depth} | ctx],
          depth
        )
