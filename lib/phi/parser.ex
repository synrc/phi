defmodule Phi.Parser do
  ...
  # Dialyzer correctly identifies that some sub-parsers always succeed (never
  # return {:error, ...}), making the defensive `err -> err` catch-all arms
  # unreachable at the type level. We keep them for safety and suppress the
  # resulting no_match warnings here.
  @dialyzer [
    {:no_match, parse_decl: 1},
    {:no_match, parse_data_constructors: 2},
    {:no_match, parse_data_constructor: 1},
    {:no_match, parse_binder: 1},
    {:no_match, parse_comma_separated: 2}
  ]
  ...
  defp parse_body(name, tokens) do
    case parse_decls(tokens, []) do
      {:ok, decls, rest} ->
        {:ok, %AST.Module{name: name, declarations: decls}, rest}
  ...
  defp parse_module_name([{:proper_name, _, _, name} | rest], acc) do
    ...
    case rest do
      [{:dot, _, _}, {:proper_name, _, _, _} | _] ->
        [{:dot, _, _} | rest2] = rest
        parse_module_name(rest2, new_acc)
  ...
  defp parse_decls(tokens, acc) do
    case parse_decl(tokens) do
      {:ok, decl, rest} ->
        # IO.puts("parse_decls MATCHED: id=#{trace_id} struct=#{decl.__struct__} rest=#{inspect(Enum.take(rest, 2))} acc_len=#{length(acc)}")
        case rest do
          [{:semicolon, _, _} | rest2] ->
            parse_decls(rest2, [decl | acc])

          [{:else_kw, _, _} | _] ->
            parse_decls(rest, [decl | acc])

          [{:right_brace, _, _} | rest2] ->
            {:ok, Enum.reverse([decl | acc]), rest2}

          [{tok, _, _} | _] when tok in [:right_paren, :right_square, :in] ->
            {:ok, Enum.reverse([decl | acc]), rest}
  ...
  # --- Declaration Rules ---
  defp parse_decl([{:semicolon, _, _} | rest]), do: parse_decl(rest)

  defp parse_decl([{:import, _, _} | rest]) do
    case parse_module_name(rest, "") do
      {:ok, mod_name, rest2} ->
        {import_list, rest3, hiding} =
          case rest2 do
            [{:left_paren, _, _} | _] ->
    ...
    case rest do
      [{:number, _, _, prec}, item | rest2] ->
        ...
        case rest2 do
          ...
          _ ->
            {:error, :invalid_fixity, rest2}
        end

      _ ->
        {:error, :invalid_fixity, rest}
    end
  end
  ...
  defp parse_decl([
         ...
         {:import, _, _},
         {:data, _, _},
         {:proper_name, _, _, name},
         {:double_colon, _, _} | rest
       ]) do
  ...
  defp parse_decl([{:newtype, _, _}, {:proper_name, _, _, name} | rest]) do
    case parse_type_vars(rest, []) do
      {:ok, vars, [{:=, _, _}, {:proper_name, _, _, constr} | rest2]} ->
        case parse_type(rest2) do
          {:ok, type, rest3} ->
            {:ok, %AST.DeclNewtype{name: name, args: vars, constructor: constr, type: type},
             rest3}
  ...
  defp parse_decl([{:class, _, _} | rest]) do
    case parse_type(rest) do
      {:ok, t, rest2} ->
        ...
        case flatten_type_app(
               case t do
                 %AST.TypeConstrained{type: inner} -> inner
                 _ -> t
               end,
               []
             ) do
            ...
            case rest3 do
              [{:where, _, _}, {:left_brace, _, _} | rest4] ->
                case parse_decls(rest4, []) do
                  {:ok, members, rest5} ->
                    {:ok, %AST.DeclClass{name: name, args: args, members: Enum.reverse(members)},
                     rest5}
              ...
              _ ->
                {:ok, %AST.DeclClass{name: name, args: args, members: []}, rest3}
            end

          _ ->
            {:error, :invalid_class_header, rest3}
        end
  ...
  defp parse_decl([{:else_kw, _, _}, {:instance, _, _} | rest]),
    do: parse_instance_with_where(rest)
    ...
    case parse_binder(tokens) do
      {:ok, binder, [{:=, _, _} | rest]} ->
        case parse_expr(rest) do
          {:ok, expr, rest2} ->
            {:ok, %AST.DeclValue{name: "_pat", binders: [binder], expr: expr}, rest2}
            ...
            case parse_expr(rest2) do
              {:ok, expr, rest3} ->
                case rest3 do
                  [{:where, _, _}, {:left_brace, _, _} | rest4] ->
                    case parse_decls(rest4, []) do
                      {:ok, local_decls, rest5} ->
                        expr_with_where = %AST.ExprLet{bindings: local_decls, body: expr}

                        {:ok, %AST.DeclValue{name: name, binders: binders, expr: expr_with_where},
                         rest5}
                ...
                case rest2 do
                  [{:where, _, _}, {:left_brace, _, _} | rest3] ->
                    case parse_decls(rest3, []) do
                      {:ok, local_decls, rest4} ->
                        # Wrap each guard body in the let block
                        guards_with_where =
                          Enum.map(guards, fn {g, b} ->
                            {g, %AST.ExprLet{bindings: local_decls, body: b}}
                          end)

                        {:ok,
                         %AST.DeclValue{name: name, binders: binders, guards: guards_with_where},
                         rest4}
                  ...
                  _ ->
                    {:ok, %AST.DeclValue{name: name, binders: binders, guards: guards}, rest2}
                end
          ...
          _ ->
            case parse_binder([{:var_ident, 0, 0, name} | rest]) do
              {:ok, binder, [{:=, _, _} | rest2]} ->
                case parse_expr(rest2) do
                  {:ok, expr, rest3} ->
                    {:ok, %AST.DeclValue{name: "_pat", binders: [binder], expr: expr}, rest3}
              ...
              _ ->
                {:error, :invalid_declaration, [{:var_ident, 0, 0, name} | rest]}
            end
        end
    end
  end
    ...
    case parse_type(tokens) do
      {:ok, type, rest} ->
        ...
        case flatten_type_app(t, []) do
          ...
          _ ->
            {:error, :invalid_instance_header, rest}
        end
  ...
  defp parse_import_list_items([{:right_paren, _, _} | rest], acc),
    do: {:ok, Enum.reverse(acc), rest}

  defp parse_import_list_items([{:comma, _, _} | rest], acc),
    do: parse_import_list_items(rest, acc)
  ...
  defp parse_import_list_items(
         [{:left_paren, _, _}, {:operator, _, _, op}, {:right_paren, _, _} | rest],
         acc
       ) do
    parse_import_list_items(rest, [{:operator, op} | acc])
  end
  ...
  defp parse_export_list_items([{:right_paren, _, _} | rest], acc),
    do: {:ok, Enum.reverse(acc), rest}

  defp parse_export_list_items([{:comma, _, _} | rest], acc),
    do: parse_export_list_items(rest, acc)
  ...
  defp parse_type_atom([{:var_ident, _, _, name} | rest]),
    do: {:ok, %AST.TypeVar{name: name}, rest}
  ...
  defp parse_type_atom([{:proper_name, _, _, name} | rest]) do
    case rest do
      ...
      _ ->
        {:ok, %AST.TypeConstructor{name: name}, rest}
    end
  end

  defp parse_type_atom([{:unit, _, _} | rest]),
    do: {:ok, %AST.TypeConstructor{name: "Unit"}, rest}

  defp parse_type_atom([{:atom, _, _, _} | rest]),
    do: {:ok, %AST.TypeConstructor{name: "Atom"}, rest}
  ...
  defp parse_type_atom([{:number, _, _, _Val} | rest]),
    do: {:ok, %AST.TypeConstructor{name: "Number"}, rest}

  defp parse_type_atom([{:left_brace, _, _} | rest]) do
    case parse_comma_separated(rest, &parse_record_field_type/1) do
      {:ok, fields, [{:right_brace, _, _} | rest2]} ->
        field_names = Enum.map(fields, fn {name, _type} -> name end)
        {:ok, %AST.TypeRecord{fields: field_names}, rest2}
  ...
  defp parse_type_vars([{:var_ident, _, _, name} | rest], acc),
    do: parse_type_vars(rest, [name | acc])
  ...
  defp parse_data_constructors(tokens, acc) do
    case tokens do
      [{:pipe, _, _} | rest] ->
        parse_data_constructors(rest, acc)
  ...
  defp parse_expr_infix(tokens) do
    case parse_expr_atom_or_app(tokens) do
      {:ok, left, rest} ->
        case rest do
          [{:operator, _, _, op} | rest2] ->
            parse_expr_infix_tail(left, {:op, op}, rest2)

          [{:backtick, _, _}, {:var_ident, _, _, op}, {:backtick, _, _} | rest2] ->
            parse_expr_infix_tail(left, {:backtick, op}, rest2)
      ...
      [{tok_type, _, _} | _] when tok_type in [:backslash, :let, :if_kw, :case, :do] ->
        case parse_expr(tokens) do
          {:ok, right, rest} ->
            {:ok,
             %AST.ExprApp{
               func: %AST.ExprApp{func: %AST.ExprVar{name: op_name}, arg: left},
               arg: right
             }, rest}
            ...
            case rest_t do
              [{:double_colon, _, _} | rest_ty] ->
                case parse_type(rest_ty) do
                  {:ok, _type, rest_ty2} ->
                    case rest_ty2 do
                      [{:operator, _, _, op2} | rest_inf] ->
                        parse_expr_infix_tail(new_left, {:op, op2}, rest_inf)

                      _ ->
                        {:ok, new_left, rest_ty2}
                    end
              ...
              [{:operator, _, _, op2} | rest_loop] ->
                parse_expr_infix_tail(new_left, {:op, op2}, rest_loop)

              [{:backtick, _, _}, {:var_ident, _, _, op2}, {:backtick, _, _} | rest_loop] ->
                parse_expr_infix_tail(new_left, {:backtick, op2}, rest_loop)
  ...
  defp parse_expr_atom_base([{:proper_name, _, _, name} | rest]) do
    case parse_module_name([{:proper_name, 0, 0, name} | rest], "") do
      {:ok, full_name, rest2} ->
        case rest2 do
          [{:dot, _, _}, {:var_ident, _, _, v} | r] ->
            {:ok, %AST.ExprVar{name: full_name <> "." <> v}, r}

          _ ->
            {:ok, %AST.ExprVar{name: full_name}, rest2}
        end
  ...
  defp parse_expr_atom_base([{:number, _, _, val} | rest]),
    do: {:ok, %AST.ExprVar{name: "num_#{val}"}, rest}

  defp parse_expr_atom_base([{:float, _, _, val} | rest]),
    do: {:ok, %AST.ExprVar{name: "float_#{val}"}, rest}

  defp parse_expr_atom_base([{:string, _, _, val} | rest]),
    do: {:ok, %AST.ExprVar{name: "str_" <> val}, rest}

  defp parse_expr_atom_base([{:char, _, _, val} | rest]),
    do: {:ok, %AST.ExprVar{name: "char_#{val}"}, rest}
  ...
  defp parse_expr_atom_base([{:left_paren, _, _} | rest]) do
    case rest do
      [{:operator, _, _, op}, {:right_paren, _, _} | rest2] ->
        {:ok, %AST.ExprVar{name: op}, rest2}
      ...
      _ ->
        case parse_comma_separated(rest, &parse_expr/1) do
          ...
          {:ok, exprs, [{:right_paren, _, _} | rest2]} ->
            {:ok, %AST.ExprTuple{elems: exprs}, rest2}

          {:ok, _, [{tok, line, col} | _]} ->
            {:error, :expected_right_paren_found, {tok, line, col}}

          {:ok, _, []} ->
            {:error, :expected_right_paren_eof}
  ...
  defp parse_expr_atom_base([{:left_square, _, _} | rest]) do
    case parse_comma_separated(rest, &parse_expr/1) do
      ...
      {:ok,
       [
         %AST.ExprApp{
           func: %AST.ExprApp{func: %AST.ExprVar{name: ".."}, arg: start_e},
           arg: end_e
         }
       ], [{:right_square, _, _} | rest2]} ->
      ...
      {:ok, exprs, [{:right_square, _, _} | rest2]} ->
        {:ok, %AST.ExprList{elems: exprs}, rest2}
  ...
  defp parse_expr_atom_tail(expr, [{:dot, _line, _col}, {:var_ident, _, _, field} | rest]) do
    # Field access: expr.field
    parse_expr_atom_tail(
      %AST.ExprApp{func: %AST.ExprVar{name: "access_#{field}"}, arg: expr},
      rest
    )
  end
  ...
  defp parse_case_branches(tokens, acc) do
    case parse_case_branch(tokens) do
      {:ok, branch, rest} ->
        case rest do
          [{:semicolon, _, _} | rest2] ->
            parse_case_branches(rest2, [branch | acc])

          [{:right_brace, _, _} | _] ->
            parse_case_branches(rest, [branch | acc])

          [{tok, line, col} | _] ->
            {:error, :expected_semicolon_or_brace_in_case, {tok, line, col}}
  ...
  defp parse_do_statements(tokens, acc) do
    case parse_do_statement(tokens) do
      {:ok, stat, rest} ->
        case rest do
          [{:semicolon, _, _} | rest2] ->
            parse_do_statements(rest2, [stat | acc])

          [{:right_brace, _, _} | rest2] ->
            {:ok, Enum.reverse([stat | acc]), rest2}

          [{tok, _, _} | _] when tok in [:right_paren, :right_square, :in] ->
            {:ok, Enum.reverse([stat | acc]), rest}

          _ ->
            parse_do_statements(rest, [stat | acc])
        end
  ...
  defp parse_record_field_type([{:var_ident, _, _, name}, {:double_colon, _, _} | rest]) do
    case parse_type(rest) do
      {:ok, type, rest2} -> {:ok, {name, type}, rest2}
      err -> err
    end
  end
  ...
  defp parse_lhs_binders([{:operator, _, _, "="} | _] = tokens, acc),
    do: {:ok, Enum.reverse(acc), tokens}

  defp parse_lhs_binders([tok | _] = tokens, acc) when elem(tok, 0) == :operator,
    do: {:ok, Enum.reverse(acc), tokens}
  ...
  defp parse_binder([{:left_paren, _, _} | rest]) do
    case rest do
      [{:operator, _, _, op}, {:right_paren, _, _} | rest2] ->
        {:ok, %AST.BinderVar{name: op}, rest2}

      _ ->
        case parse_comma_separated(rest, &parse_binder/1) do
          {:ok, [b], [{:right_paren, _, _} | rest2]} ->
            {:ok, b, rest2}

          {:ok, binders, [{:right_paren, _, _} | rest2]} ->
            {:ok, %AST.BinderTuple{elems: binders}, rest2}
    ...
    case parse_comma_separated(rest, &parse_binder/1) do
      {:ok, binders, [{:pipe, _, _} | rest_p]} ->
        case parse_binder(rest_p) do
          {:ok, tail, [{:right_square, _, _} | rest2]} ->
            {:ok, %AST.BinderList{head: binders, tail: tail}, rest2}

          err ->
            err
        end

      {:ok, [], [{:right_square, _, _} | rest2]} ->
        {:ok, %AST.BinderList{head: nil, tail: nil}, rest2}

      {:ok, binders, [{:right_square, _, _} | rest2]} ->
        {:ok, %AST.BinderList{head: binders, tail: nil}, rest2}
