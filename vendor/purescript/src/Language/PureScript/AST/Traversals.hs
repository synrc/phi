...
everywhereOnValues f g h = (f', g', h')
  where
    ...
    h' (ListBinder xs x) = h (ListBinder (fmap h' xs)  (h' x))
    h' other = h other
    ...
    handleCaseAlternative ca =
      ca
        { caseAlternativeBinders = fmap h' (caseAlternativeBinders ca),
...
everywhereOnValuesTopDownM f g h = (f' <=< f, g' <=< g, h' <=< h)
  where
    ...
    h' (ListBinder xs x) = ListBinder <$> traverse (h' <=< h) xs  <*> (h x >>= h')
    h' other = h other
...
everywhereOnValuesM f g h = (f', g', h')
  where
    ...
    h' (ListBinder xs x) = (ListBinder <$> traverse h' xs <*> h' x) >>= h
    h' other = h other
    handleCaseAlternative :: CaseAlternative -> m CaseAlternative
    handleCaseAlternative (CaseAlternative bs val) =
      CaseAlternative
...
everythingOnValues (<>.) f g h i j = (f', g', h', i', j')
  where
    ...
    h' b@(ListBinder  xs x) = foldl (<>.) (h b) (fmap h' (xs ++ [x]))
    h' b = h b
...
everythingWithContextOnValues s0 r0 (<>.) f g h i j = (f'' s0, g'' s0, h'' s0, i'' s0, j'' s0)
  where
    ...
    h' s (ListBinder xs x) = foldl (<>.) r0 (fmap (h'' s) (xs ++ [x]))
    h' _ _ = r0
...
everywhereWithContextOnValuesM s0 f g h i j = (f'' s0, g'' s0, h'' s0, i'' s0, j'' s0)
  where
    ...
    h' s (ListBinder xs x) = ListBinder <$> traverse (h'' s) xs <*> h'' s x
...
everythingWithScope f g h i j = (f'', g'', h'', i'', \s -> snd . j'' s)
  where
    ...
    h' s (ListBinder xs x) = foldMap (h'' s) (xs ++ [x])
    h' _ _ = mempty
